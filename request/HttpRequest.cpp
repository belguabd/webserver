...
getMatchedLocationUpload(const std::string &path,
  ...
  if (configUploads.find(keyLocationUpload) == configUploads.end())
    configUploads[keyLocationUpload] =
        (LocationConfig){._upload_store = UPLOAD_FOLDER,
...
int HttpRequest::setDataCgi(string data, ServerConfig &config,
...
int HttpRequest::defineTypeMethod(string firstline) {
  ...
  for (size_t i = 0; i < firstline.length(); i++) {
    if (firstline[0] == ' ' || firstline[i] == '\t' ||
        ...
        (firstline[i] == ' ' &&
         (firstline[i + 1] == '\0' || firstline[i + 1] == '\r' ||
          firstline[i + 1] == '\n'))) {
...
bool isNonASCII(char c) { return static_cast<unsigned char>(c) > 127; }
...
int HttpRequest::parseFiledLine(std::string &filedLine) {
  ...
  if ((pos = filedLine.find(":")) == std::string::npos ||
      std::find_if(filedLine.begin(), filedLine.end(), isNonASCII) !=
          filedLine.end())
    return 400;

  std::string filedLineName = filedLine.substr(0, pos);
  while (filedLineName[0] == ' ' || filedLineName[0] == '\t')
    filedLineName.erase(filedLineName.begin()); // remove spaces in the
                                                // beginning
  ...
  for (size_t i = 0; i < filedLineName.size(); i++) {
    filedLineName[i] = std::toupper(filedLineName[i]);
    if (filedLineName[i] == '-')
      filedLineName[i] = '_';
  }
  ...
  if (mapheaders.find(filedLineName) != mapheaders.end()) {
    ...
    keys.push_back("HOST");
    keys.push_back("CONTENT_LENGTH");
    keys.push_back("TRANSFER_ENCODING");
